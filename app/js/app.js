...
function primaryShow(title){
  ...
  $.when(ajaxTemplate).done(()=> {
    ...
    ajaxTemplate = ajaxTemplate
                    ...
                    .replace("##score##", title.rating.average)
                    .replace("##cover##", title.image.medium)
                    .replace("##genres##", genres)
                    .replace("##coverprime##", title.image.original)
                    .replace("##description##", title.summary)
                    .replace("##runtime##", title.runtime)
                    .replace("##lang##", title.language);
...
function runGrid(titles){
  ...
  $.when(ajaxTemplate).done(()=> {
    for (var title = 0; title < titles.length; title++) {
      ...
      ajaxTemplate = template;
      ajaxTemplate = ajaxTemplate
                      .replace("##title##", titles[title].name)
                      .replace("##score##", titles[title].rating.average)
                      .replace("##cover##", titles[title].image.medium)
                      .replace("##genres##", genres);
      $node.append(ajaxTemplate);
    }
  });
